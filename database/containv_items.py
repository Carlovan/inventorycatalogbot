...
class DbContainvItems(Database):
	...
	def get(self, user):
		assert(type(user) is utils.user.User)
		sql = 'SELECT * FROM containv_items INNER JOIN items ON itemid = id WHERE userid = %s;'
	...
	def exists(self, item, user):
		...
		assert(type(user) is utils.user.User)
		sql = 'SELECT * FROM containv_items WHERE itemid = %s AND userid = %s;'
