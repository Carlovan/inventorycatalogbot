...
def run(bot, update, args):
	...
	if user.state == UserState.NONE:
		...
		user.state = UserState.CONFRONTAINV_A
		dbusers.update(user)
		update.message.reply_text('Ora mandami il primo inventario da confrontare. Quando hai finito usa /fine. Per cancellare /annulla.')
	elif user.state == UserState.CONFRONTAINV_A or user.state == UserState.CONFRONTAINV_B:
		user.other = ' '.join(args)
		dbusers.update(user)
